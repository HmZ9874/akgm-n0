"""Append-only rejection memory for synthesized micro-state semantics."""
...
from akgm_n0.learner.semantic_slot import MicroProgram, micro_program_key


class MicroMistakeLibraryError(ValueError):
    """Raised when the micro-mistake hash chain is invalid."""


@dataclass(frozen=True, slots=True)
class MicroMistakeRecord:
    mistake_id: str
    failed_scope: str
    condition_key: str
...
class MicroMistakeLibrary:
    ...
    def __init__(self, path: Path, *, clock: Callable[[], datetime] | None = None):
        ...
        self._records: list[MicroMistakeRecord] = []
        if self.path.exists():
            self._load_and_verify()

    @property
    def records(self) -> tuple[MicroMistakeRecord, ...]:
        return tuple(self._records)

    def find_equivalent(
        self, program: MicroProgram, *, failed_scope: str, condition_key: str
    ) -> tuple[MicroMistakeRecord, ...]:
        signature = micro_program_key(program)
        return tuple(
            record
    ...
    def record(
        ...
        *,
        ...
        if not failed_scope or not condition_key or not counterexamples:
            raise ValueError("micro mistake requires scope, condition, and evidence")
        ...
        if existing:
            return existing[0]
        signature = micro_program_key(program)
        mistake_id = "MM-" + hashlib.sha256(
            f"{failed_scope}|{condition_key}|{signature}".encode("utf-8")
        ).hexdigest()[:16]
        timestamp = self._clock()
        if timestamp.tzinfo is None:
            raise MicroMistakeLibraryError("mistake clock must be timezone-aware")
        event: dict[str, Any] = {
            "schema_version": "micro-mistake-event-v0.1",
        ...
        with self.path.open("r", encoding="utf-8") as stream:
            for line_number, line in enumerate(stream, start=1):
                ...
                try:
                    ...
                    raise MicroMistakeLibraryError(
                        f"invalid JSON at line {line_number}"
                    ) from exc
                if event.get("event_index") != len(self._events):
                    raise MicroMistakeLibraryError("event index mismatch")
                if event.get("previous_event_hash") != previous_hash:
                    raise MicroMistakeLibraryError("hash chain mismatch")
                if event.get("event_hash") != _event_hash(event):
                    raise MicroMistakeLibraryError("event hash mismatch")
                self._events.append(event)
                self._records.append(_record_from_event(event))
                previous_hash = event["event_hash"]


def _record_from_event(event: Mapping[str, Any]) -> MicroMistakeRecord:
    return MicroMistakeRecord(
