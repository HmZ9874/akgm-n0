...
from akgm_n0.learner.rational_algebra_frontier import RationalAlgebraSemantic
from .rational_algebra_proof import verify_rational_algebra_semantic
class RationalAlgebraRoom:
    ZERO_HASH="0"*64
    ...
    def record(self,s:RationalAlgebraSemantic,p:Mapping[str,Any])->Mapping[str,Any]:
        r=verify_rational_algebra_semantic(s)
        if not r["passed"] or dict(p)!=r:raise ValueError("rational algebra proof cannot replay")
        old=next((x for x in self._events if x["semantic"]["semantic_id"]==s.semantic_id),None)
        if old:return old
        e={"schema_version":"rational-algebra-event-v0.1","event_index":len(self._events),"timestamp":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"semantic":s.to_dict(),"proof":r,"previous_event_hash":self._events[-1]["event_hash"] if self._events else self.ZERO_HASH};e["event_hash"]=_h(e);self.path.parent.mkdir(parents=True,exist_ok=True)
    ...
    def _load(self)->None:
        ...
        for n,line in enumerate(self.path.read_text(encoding="utf-8").splitlines(),1):
            ...
            e=json.loads(line)
            if e.get("event_index")!=len(self._events) or e.get("previous_event_hash")!=prev or e.get("event_hash")!=_h(e):raise ValueError(f"rational room chain mismatch {n}")
            s=RationalAlgebraSemantic.from_dict(e["semantic"]);p=verify_rational_algebra_semantic(s)
            if not p["passed"] or e.get("proof")!=p:raise ValueError("stored rational proof cannot replay")
